    ...
    <div class="min-h-screen bg-slate-950 pb-24 lg:pb-8" x-data="{}" x-cloak>
        ...
        <div class="lg:ml-[72px]">
                ...
                <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-6 sm:pt-8 sm:pb-8">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white">Lokasi Tersimpan</h1>
                                <p class="text-slate-400 text-sm mt-0.5">Kelola lokasi untuk pemantauan risiko bencana</p>
                            </div>
                        </div>
                        <a href="{{ route('locations.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-green-500 text-white text-sm font-semibold rounded-full hover:shadow-lg hover:scale-[1.02] transition-all duration-300">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
                ...
                @if($locations->count() > 0)
                    <div class="grid gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
                        @foreach($locations as $location)
                            <div class="glass-dark border border-white/5 rounded-2xl p-5 shadow-card relative group hover:bg-white/[0.06] transition-all duration-300">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex items-center gap-3 w-full">
                                        <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center border border-white/10 shrink-0 shadow-inner">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2 mb-1">
                                                <h3 class="font-bold text-white text-base truncate">{{ $location->name }}</h3>
                                                @if($location->is_default)
                                                    <span class="shrink-0 bg-gradient-to-r from-emerald-500 to-green-500 text-white text-[10px] leading-tight font-bold uppercase tracking-wider px-2 py-0.5 rounded shadow-soft">
                                                        Default
                                                    </span>
                                                @endif
                                            </div>
                                            <p class="text-xs text-slate-400 line-clamp-2 leading-relaxed">{{ $location->address ?? 'Alamat tidak tersedia' }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white/[0.03] rounded-xl p-3 mb-4 space-y-2">
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="text-slate-400">Notifikasi</span>
                                        <span class="{{ $location->notifications_enabled ? 'text-emerald-400' : 'text-slate-500' }}">
                                            {{ $location->notifications_enabled ? 'Aktif' : 'Nonaktif' }}
                                        </span>
                                    </div>
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="text-slate-400">Radius</span>
                                        <span class="text-white font-medium">{{ $location->notification_radius_km }} km</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-2">
                                    @if(!$location->is_default)
                                        <form action="{{ route('locations.set-default', $location) }}" method="POST" class="flex-1">
                                            @csrf
                                            <button type="submit" class="w-full py-2 px-3 bg-white/[0.05] hover:bg-white/10 text-slate-300 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-1.5">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                                Jadikan Default
                                            </button>
                                        </form>
                                    @else
                                        <div class="flex-1 py-2 px-3 bg-emerald-500/10 text-emerald-400 text-sm font-medium rounded-lg text-center">
                                            Lokasi Default
                                        </div>
                                    @endif

                                    <a href="{{ route('locations.edit', $location) }}" class="p-2 bg-white/[0.05] hover:bg-white/10 text-slate-300 rounded-lg transition-colors" title="Edit">
                                    ...
                                    <form action="{{ route('locations.destroy', $location) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus lokasi ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 bg-white/[0.05] hover:bg-rose-500/10 text-slate-300 hover:text-rose-400 rounded-lg transition-colors" title="Hapus">
                ...
                @else
                    {{-- Empty State --}}
                    <div class="text-center py-16 glass-dark rounded-2xl shadow-card border border-white/5">
                        ...
                        <h3 class="text-lg font-bold text-white mb-2">Belum Ada Lokasi Tersimpan</h3>
                        <p class="text-slate-400 mb-6 max-w-md mx-auto">Tambahkan lokasi (rumah, kantor, kos, dll) untuk memantau risiko bencana dan mendapatkan notifikasi.</p>
                        <a href="{{ route('locations.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-green-500 text-white text-sm font-semibold rounded-full hover:shadow-lg hover:scale-[1.02] transition-all duration-300">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
