<x-app-layout>
    ...
    {{-- Guide Category — Fluid Modern Dark Dashboard Style --}}
        ...
        <div class="lg:ml-[72px]">
                ...
                <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-5 pb-6 sm:pt-6 sm:pb-8">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex items-center gap-4" data-aos="fade-right" data-aos-delay="200">
                            <div class="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg text-2xl
                                @if($currentCategory === 'earthquake') bg-gradient-to-br from-orange-400 to-amber-500 shadow-amber-500/20
                                @elseif($currentCategory === 'flood') bg-gradient-to-br from-sky-400 to-blue-500 shadow-sky-500/20
                                @elseif($currentCategory === 'landslide') bg-gradient-to-br from-amber-600 to-yellow-700 shadow-amber-600/20
                                @elseif($currentCategory === 'tsunami') bg-gradient-to-br from-cyan-400 to-teal-500 shadow-cyan-500/20
                                @elseif($currentCategory === 'fire') bg-gradient-to-br from-rose-400 to-red-500 shadow-rose-500/20
                                @elseif($currentCategory === 'volcano') bg-gradient-to-br from-red-500 to-rose-700 shadow-red-500/20
                                @else bg-gradient-to-br from-emerald-500 to-green-600 shadow-emerald-500/20
                                @endif">
                                @if($currentCategory === 'earthquake') 🌍
                                @elseif($currentCategory === 'flood') 🌊
                                @elseif($currentCategory === 'landslide') ⛰️
                            ...
                            <div>
                                ...
                                <p class="text-slate-400 text-sm mt-0.5">{{ $guides->total() }} panduan mitigasi tersedia</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-8">
                    ...
                    <div data-aos="fade-up" data-aos-delay="300">
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('guides.index') }}"
                               class="px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 glass border border-white/5 text-slate-300 shadow-card hover:bg-white/10 hover:text-white hover:scale-[1.02] active:scale-[0.98]">
                                Semua
                            </a>
                            @foreach($categories as $category)
                                <a href="{{ route('guides.category', $category) }}"
                                   class="px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 {{ $currentCategory === $category ? 'bg-gradient-to-r from-emerald-500 to-green-500 text-white shadow-soft border-transparent' : 'glass border border-white/5 text-slate-300 shadow-card hover:bg-white/10 hover:text-white' }} hover:scale-[1.02] active:scale-[0.98] capitalize">
                ...
                @if($guides->count() > 0)
                    <div class="grid gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
                        @foreach($guides as $index => $guide)
                            <a href="{{ route('guides.show', $guide->slug) }}"
                               class="tilt-card card-fluid glass-dark border border-white/5 rounded-2xl shadow-card overflow-hidden group flex flex-col transition-all duration-300 transform"
                               ...
                               data-tilt data-tilt-glare data-tilt-max-glare="0.2" data-tilt-scale="1.02">
                                ...
                                @else
                                    <div class="h-32 bg-slate-800/50 relative flex items-center justify-center overflow-hidden border-b border-white/5">
                                        <svg class="w-12 h-12 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                                        </svg>
                                        <div class="absolute -bottom-4 -right-4 w-20 h-20 bg-emerald-500/10 rounded-full blur-xl"></div>
                                    </div>
                                @endif
                    ...
                    {{-- Pagination --}}
                    @if($guides->hasPages())
                        <div class="mt-8 glass-dark p-4 rounded-2xl border border-white/5" data-aos="fade-up">
                            {{ $guides->links() }}
                        </div>
                    @endif
                @else
                    ...
                    <div class="text-center py-16 glass-dark rounded-2xl shadow-card border border-white/5" data-aos="fade-up">
                        ...
                        <p class="text-slate-400">Belum ada panduan mitigasi untuk kategori ini.</p>
                    </div>
                @endif
            </div>
        </div>
